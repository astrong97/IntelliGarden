...
public class Display extends FlowPane {
	...
	public Display() {
		GaugeBuilder builder = GaugeBuilder.create().skinType(Gauge.SkinType.SIMPLE_SECTION)
				.barBackgroundColor(Color.GREY).animated(true).animationDuration(1000);
		...
		co2G.setThresholdColor(Color.GREEN);
		co2G.setThresholdVisible(true);

		VBox vbox_tempValues = getVBox("TEMPERATURE", Color.GREEN, tempG);
		VBox vbox_humidValues = getVBox("HUMIDITY", Color.GREEN, humidG);
		VBox vbox_moistValues = getVBox("MOISTURE", Color.GREEN, moistG);
		VBox vbox_phValues = getVBox("PH", Color.GREEN, phG);
		VBox vbox_co2Values = getVBox("CO2", Color.GREEN, co2G);
...
//		vbox_sensorValues.getChildren().addAll(lbl_currentHeading, lbl_temperature, lbl_humidity, lbl_moisture, lbl_PH, lbl_CO2);
		...
		setHgap(100);
		vbox_desiredValues.getChildren().addAll(lbl_desiredHeading, lbl_desiredTemp, lbl_desiredHumid, lbl_desiredMoist,
				lbl_desiredPH, lbl_desiredCO2);
		getChildren().add(vbox_desiredValues);
		vbox_hardwareStatus.getChildren().addAll(lbl_hardwareHeading, lbl_AirCond, lbl_Humidifier, lbl_Irrigation,
				lbl_Ventilator, lbl_CO2release);
